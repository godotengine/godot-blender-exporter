...
from .visual_shader import export_visual_shader
...
def export_material(escn_file, export_settings, material):
    """Exports blender internal/cycles/eevee material as best it can"""
...
def generate_material_resource(escn_file, export_settings, material):
    ...
    if (engine in ('CYCLES', 'BLENDER_EEVEE') and
        ...
        gd_shader_mtl = InternalResource("ShaderMaterial", material_rsc_name)
        try:
            if export_settings['feature_visual_shader']:
                export_visual_shader(
                    escn_file, export_settings, material, gd_shader_mtl
                )
            else:
