...
def export_script_shader(escn_file, export_settings, bl_node_mtl, gd_mtl):
    """Export cycles material to godot export_script_shader shader"""
    shader_globals = ShaderGlobals()
    fragment_shader = shader_globals.fragment_shader
    vertex_shader = shader_globals.vertex_shader

    mat_output_node = find_material_output_node(bl_node_mtl.node_tree)
    ...
    for image, uniform_var in shader_globals.textures.items():
        ...
        gd_mtl[shader_param_key] = "ExtResource({})".format(resource_id)
